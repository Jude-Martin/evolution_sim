...
def write_novel_stop_events(record_dir, generation, variants):
    """
    Write one row per novel stop event, not just the first novel stop per variant.
    """
    output_path = os.path.join(record_dir, f"gen_{generation:03d}_novel_stop_events.tsv")
    ...
    with open(output_path, "w", newline="") as handle:
        ...
        for variant in variants:
            for stop in variant.get("novel_stop_events", []):
                writer.writerow([
                    generation,
                    variant["simulation_index"],
                    variant["source_parent_id"],
                    stop["stop_codon"],
                    stop["codon_start_1based"],
                    stop["codon_index_global_1based"],
...
def main():
    ...
    for generation in range(1, num_generations + 1):
        ...
        for variant in all_variants:
            for stop in variant.get("novel_stop_events", []):
                stop_counter[stop["stop_codon"]] += 1
        ...
        if checkpoint_written:
            ...
            write_novel_stop_events(record_dir, generation, all_variants)
